...
class FormInput extends Component<{navigate:(str : string, any : any) => void}, FormInputState> {
    ...
    if (!loading) {
      ...
      if (str === "id") {
        ...
        try {
          ...
          console.log(data);
          this.props.navigate("/details", { state: data.near_earth_objects[
            Math.floor(Math.random() * data.near_earth_objects.length)] });
        } catch (error) {
          console.error("Error fetching asteroid data:", error);
          alert("Error fetching asteroid data! Please recheck the input ID");
  ...
  render() {
    ...
    return (
      <div className="flex flex-col bg-cover justify-center items-center h-[100vh] text-white pt-24 overflow-clip bg-center w-full h-auto bg-center-bottom bg-no-repeat bg-cover">
        <div className="z-10 top-6 absolute">
          <p className="font-bold text-4xl ">AsteroidOpedia</p>
        </div>
        <img src={bg} alt="bg" className="z-0 absolute top-0 right-0 w-full h-auto bg-center-bottom bg-no-repeat bg-cover" />
