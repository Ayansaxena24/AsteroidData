...
describe("FormInput", () => {
  ...
  const wrapper = mount(
    ...
    const getSpy = jest.spyOn(FormInput.prototype, 'fetchData');
    const search = FormInput.prototype.fetchData.mock.calls[0];

    wrapper.update();
    expect(spyWasCalled).toHaveBeenCalled();
    expect(spyWasCalled.mockResolvedValue).toEqual(mockApiResponse);
