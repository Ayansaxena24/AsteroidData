...
describe("FormInput", () => {
  ...
  const wrapper = mount(
    ...
    // Wait for the fetchData method to be called
  await Promise.resolve();

  wrapper.update();

  // Check if fetchData method was called
  expect(spyWasCalled).toHaveBeenCalledWith(expect.stringContaining('2000719'));

  // Access the component instance and check its state
  const formInputInstance = wrapper.find(FormInput).instance() as <FormInput></FormInput>;
