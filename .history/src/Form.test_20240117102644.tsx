...
describe("FormInput", () => {
  ...
  it('handles successful API fetch', async () => {
  ...
  const spyDidMount = jest.spyOn(FormInput.prototype,"fetchData");

    // Mock the fetch function
    const spyjest.spyOn(global, 'fetch').mockResolvedValue({
      ok: true,
    ...
    act(() => {
      // Simulate user input or form submission triggering API call
      wrapper.find('input').simulate('change', {
        target: { value: '2000719' },
      });
...
// Access the component instance and check its state
const formInputInstance = wrapper.find(FormInput).instance();
expect(formInputInstance.state.asteroidDetails).toEqual(mockApiResponse);
