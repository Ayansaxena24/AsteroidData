...
describe("FormInput", () => {
  ...
  await act (async () => {
    await wrapper.setState({ id: "2000719" });
  });


  await wrapper.instance.fetchData({id: "2000719"});

  expect (mockNavigation.navigate).toHaveBeenCalledWith("/details", {state: mockApiResponse.near_earth_objects[Math.floor(Math.random() * mockApiResponse.near_earth_objects.length)]}
